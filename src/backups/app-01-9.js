...
app.post("/webhookwhatsapp", async function (request, response) {
	console.log('LLEGO ESTO', request.body)
	try {
		...
		if (messageDetails) {
			...
			handleIncomingMessage(chatId, messageDetails);
...
async function handleIncomingMessage(chatId, message) {
	// Si chatId no está en chatStates, lo establecemos con el estado inicial
    if (!chatStates.has(chatId)) {
        chatStates.set(chatId, STATES.INITIAL);
    }

    const currentState = getCurrentState(chatId);  // Ahora puedes obtener el estado actual
