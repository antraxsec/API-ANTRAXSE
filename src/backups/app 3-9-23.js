...
const ESTADOS = {
	...
	SUB_FORMAENTREGA:"subMenuFormaEntrega"
    // ... otros estados
};
...
async function procesarMensajeEntrante(chatId, message) {
	...
	switch (estadoActual) {
		case ESTADOS.INICIAL:
			await estadoInicial(chatId, message, numero, tipo);
			break;
		case ESTADOS.ADMIN:
		...
		case ESTADOS.SUB_FORMAENTREGA:
			nivelSubMenuUbicacion(chatId, message, numero, tipo) 
			break;
		default:
...
async function estadoInicial(chatId, message, numero, tipo) {
	...
	if (tipo === 'text') {
		...
		// 	chatStates.set(chatId, "menu");
		// }
		// else if (mensajeInicial.some(phrase => mensajeTexto.indexOf(phrase.toLowerCase()) !== -1)) {
		// 	await mensajeFacebook(numero, "¡Hola! 🤗 Bienvenido a Multilaptops");
		// }
	} 
...
async function nivelMenu(chatId, message, numero, tipo) {
	...
	await mensajeFacebook(numero, "Estamos en el nivel del menu de opciones");
	...
	if (response.type === 'interactive' && response.interactive.type === 'list_reply') {
		...
		switch (buttonId) {
			case "btn_subMenuCatalogo":
				promocionFlow(numero, false) 
				chatStates.set(chatId, "menu");
				break;
			case "btn_subMenuComprar":
				reenviarUbicacion(numero, false) 
				chatStates.set(chatId, "menu");
				break;
			case "btn_subMenuPromocion":
				promocionFlow(numero, false) 
				chatStates.set(chatId, "menu");
				break;
			case "btn_subMenuFormaEntrega":
				await nivelSubMenuFormaEntrega(numero)
				// reenviarUbicacion(numero, false) 
				// chatStates.set(chatId, "menu");
				break;
...
async function nivelSubMenuFormaEntrega(chatId, message, numero, tipo) {

	await mensajeFacebook(numero, "Estamos en el subnivel de menu-ubicaciones");
	...
	if (response.type === 'interactive' && response.interactive.type === 'button_reply') {
		...
		switch (buttonId) {
			case "btn_submenuFormaEntrega1":
				reenviarFormasEntrega(numero, false) 
				chatStates.set(chatId, "menu");
				break;
...
async function submenuFormaEntrega(numero) {
	const texto = [
	...
	const opciones = [
		{
			...
			"rows": [
				{
					"id": "btn_subMenuCatalogo",
					"title": "Cátalogo de productos",
				},
				{
					"id": "btn_subMenuComprar",
					"title": "Comprar un producto",
				},
				{
					"id": "btn_subMenuPromocion",
					"title": "Promociones",
				},
				{
					"id": "btn_subMenuFormaEntrega",
					"title": "Formas de entregas",
				},
				{
					"id": "btn_subMenuAsesor",
