...
app.post("/webhookwhatsapp", function (request, response) {
  try {
    ...
    if (messageDetails) {
      ...
      console.log("hhhhh", messageDetails);
...
async function nivelFormaEntrega(chatId, message, numero, tipo) {
  ...
  if (
    response.type === "interactive" &&
    response.interactive.type === "button_reply"
  ) {
  ...
  if (
    ...
    switch (buttonId) {
      ...
      case "btn_submenuAsesor":
        await mensajeFacebook(
          numero,
          "Si tienes alguna pregunta o inquietud, no dudes en contactarnos llamando al siguiente número 👇"
        );
  ...
  if (response.text && response.text.body) {
    try {
      ...
      if (respuestaJSON && respuestaJSON.estado) {
        ...
        await mensajeFacebook(
          numero,
          `Muchas gracias por programar tu visita a nuestra sucursal. Estamos emocionados de conocerte y ayudarte a encontrar el producto que necesitas.`
        );
        await mensajeFacebook(
          numero,
          `Recuerda que si necesitas reprogramar tu visita por cualquier imprevisto, puedes hacerlo sin costo alguno. Sin embargo, lamentamos informarte que no podemos reservar equipos, ya que estos se pueden agotar.`
        );
        await mensajeFacebook(
          numero,
          `Te esperamos en nuestra sucursal el día y la hora que hayas programado. ¡Esperamos poder ayudarte!`
        );
    ...
    switch (buttonId) {
      case "btn_submenuMenu":
        await mensajeFacebook(
          numero,
          "Nuestro menú de opciones está disponible para que interactúes con nosotros de manera fácil y rápida."
        );
        await menuLista(numero);
        chatStates.set(chatId, "menu");
        break;
      ...
      case "btn_submenuSalir":
        await mensajeFacebook(
          numero,
          "Ha sido un placer atenderte. ¡Muchas gracias por tu preferencia!"
        );
        chatStates.set(chatId, "initial");
        break;
      default:
...
async function reenviarFormasEntrega(contactId, isReflow = false) {
  ...
  const imagen =
    "https://multilaptops.net/recursos/imagenes/tiendaonline/mapa-uyustus3.jpg";
  const texto = [
  ...
  if (response.text && response.text.body) {
    try {
      ...
      if (respuestaJSON && respuestaJSON.estado) {
        ...
        await mensajeFacebook(
          numero,
          `Nos alegra saber que has elegido nuestro servicio de entrega a domicilio para recibir tus productos. Estamos comprometidos a brindarte la mejor experiencia posible, y tu satisfacción es nuestra prioridad.`
        );
        await mensajeFacebook(
          numero,
          `En breve nos contactaremos contigo para coordinar la entrega de tu pedido.`
        );
...
async function menuEnvioNacional(numero) {
  const texto = [`Porfavor selecciona una opción`];
  const botones = [
...
async function analisisAI(datos) {
  ...
  const mensajes = [
    {
      role: "system",
      content: `Tu respuesta debe ser en formato JSON con los siguientes parametros: ${datos.parametros}. Tu funcion solo es cumplir esta orden: "${datos.objetivo}". El parametro "estado" debe ser TRUE si los datos se encuentran, FALSE en caso contrario. Agrega tu comentario en el parametro "comentario".`,
    },
...
async function asistenteAI(datos) {
  ...
  const productos = [
    ...
    },
    {
      ...
      precio: "Bs. 11600.00",
      pantalla:
        '13.3" FHD AMOLED Display (1920 x 1080) con micro bordes Touch Screen Panel',
  ...
  function descripcionDeProductos() {
    return productos
      .map(
        (producto, index) =>
          `Producto SKU: ${producto.sku}: ${producto.marcas}, RAM: ${producto.ram}, Precio: ${producto.precio}, Pantalla: ${producto.pantalla}, Procesador: ${producto.procesador}, Almacenamiento: ${producto.UnidaEstadoSolidoSSD}`
      )
      .join("\n");
  }
  ...
  const mensajes = [
    {
      role: "system",
      content: `Eres un asistente de ventas de laptops. Estos son los productos disponibles:\n${descripcionDeProductos()}\n Separa tus comentarios en parrafos. Puedes proporcionar información detallada y ayudar con la compra.`,
    },
    { role: "user", content: datos.mensaje },
  ];
  historial.push(...mensajes);
...
async function promocionFlow(contactId, isReflow = false) {
  ...
  await mensajeFacebook(
    contact,
    `¡Hola! Puedes encontrar todos los detalles y realizar la compra de la Samsung Galaxy Book en nuestra tienda 👉 multi.bz/samsung 🛒💻.`
  );
...
async function reenviarProcesoCompra(contactId, isReflow = false) {
  ...
  const imagen42 =
    "https://multilaptops.net/recursos/imagenes/tiendaonline/procesocompra-2/4.3.webp";
  const texto42 = [
    `Selecciona tu método de entrega preferido:`,
    ``,
  ...
  const imagen5 =
    "https://multilaptops.net/recursos/imagenes/tiendaonline/procesocompra-2/5.webp";
  const texto5 = [
  ...
  const imagen6 =
    "https://multilaptops.net/recursos/imagenes/tiendaonline/procesocompra-2/6.webp";
  const texto6 = [
...
async function reenviarFormasPago(contactId, isReflow = false) {
  ...
  const imagen1 =
    "https://multilaptops.net/recursos/imagenes/tiendaonline/formaspago-2/1.jpg";
  const texto1 = [
    ``,
  ...
  const imagen2 =
    "https://multilaptops.net/recursos/imagenes/tiendaonline/formaspago-2/2.jpg";
  const texto2 = [
    ``,
  ...
  const imagen3 =
    "https://multilaptops.net/recursos/imagenes/tiendaonline/formaspago-2/3.jpg";
  ...
  const imagen4 =
    "https://multilaptops.net/recursos/imagenes/tiendaonline/formaspago-2/4.jpg";
  const texto4 = [
    ``,
    `*Efectivo:*`,
  ...
  await mensajeFacebook(
    contactId,
    `Si tienes cualquier consulta, ¡estamos a tu disposición para ayudarte!`
  );
}
...
async function asistenteGPT(contactId, isReflow = false, message) {
  ...
  async function chatConUsuario(mensajeDelUsuario) {
    const mensajeInicial = [
      {
        role: "system",
        content: `Eres un vendedor de laptops samsung, solo vende estos disponibles:\n${descripcionDeProductos()}\nProporciona varios modelos.`,
      },
...
async function asistenteGPTxxx(mensaje, isReflow = false, contact, message) {
  ...
  function descripcionDeProductos() {
    return productos
      .map(
        (producto, index) =>
          `Producto con codigo SKU: ${producto.sku}: ${producto.marcas}, RAM: ${producto.ram}, Precio: ${producto.precio}, Pantalla: ${producto.pantalla}, Procesador: ${producto.procesador}, Unidad Estado Sólido: ${producto.UnidaEstadoSolidoSSD}`
      )
      .join("\n");
  }
...
///funcion para mnadar productos por SKU
async function MandarPorSKU(data) {
